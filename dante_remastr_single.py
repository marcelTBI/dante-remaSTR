...
def write_phased_predictions(
    phased_sequences: dict, output: str
) -> None:
    lines = []
    lines.append(f"{phased_sequences['motif_name']}\t{''.join(phased_sequences['nomenclature1'])}\t{phased_sequences['errors1']}\n")
    lines.append(f"{phased_sequences['motif_name']}\t{''.join(phased_sequences['nomenclature2'])}\t{phased_sequences['errors2']}\n")
...
def write_vcf(df: pd.DataFrame, out: str) -> None:
    lines = [
        ...
        '##INFO=<ID=RU,Number=1,Type=String,Description="Repeat unit in ref orientation">\n',
        '##INFO=<ID=SVLEN,Number=1,Type=Integer,Description="Alt length - Ref length">\n',
