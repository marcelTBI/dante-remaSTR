...
def main() -> None:
    ...
    for motif_table in generate_groups(args.input_tsv):
        ...
        # from pprint import pprint
        # print("motif_table.iloc[0, :]")
        # pprint(dict(motif_table.iloc[0, :]))
        # print("motif")
        # pprint(vars(motif))
        # print("annotations[0]")
        # pprint(vars(annotations[0]))
        # return

        all_motifs.append(motif)
        all_annotations.append(annotations)

        del motif_table, motif, annotations

    all_genotypes = []
    all_haplotypes = []
    for (motif, anns) in zip(all_motifs, all_annotations):
        genotypes = genotype_group(motif, anns)
        haplotypes = phase_group(motif, anns)
        all_genotypes.append(genotypes)
        all_haplotypes.append(haplotypes)
        del motif, anns, genotypes, haplotypes
